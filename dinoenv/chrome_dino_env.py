...
class ChromeDinoEnv(gym.Env):
    ...
    def __init__(self, render, accelerate, autoscale):
        ...
        self.observation_space = spaces.Box(
            low=0, high=255, shape=(150, 600, 4), dtype=np.uint8
        )
    ...
    def set_parameter(self, key, value):
        self.game.set_parameter(key, value)

    def set_default_parameters(self):
        print("Setting default parameters...")
        self.game.set_parameter('config.ACCELERATION', 0)
        self.game.set_parameter('config.BG_CLOUD_SPEED', 0.0)
        self.game.set_parameter('config.CLOUD_FREQUENCY', 0.0)
        self.game.set_parameter('config.INVERT_DISTANCE', 1000000000)
        self.game.set_parameter('config.MAX_CLOUDS', 0)
